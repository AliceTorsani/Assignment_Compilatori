    ...
    void processLoopSiblings(ArrayRef<Loop *> Loops, ScalarEvolution &SE) {
        ...
        for (unsigned i = 0; i+1 < Loops.size(); ++i) {
            ...
            if (isa<SCEVCouldNotCompute>(TC0) || isa<SCEVCouldNotCompute>(TC1)) {
                ...
                // SCEV canonizza le espressioni. Se sono matematicamente identiche, 
                // spesso condividono lo stesso esatto puntatore in memoria.
                if (TC0 == TC1) {
                    sameTripCount = true;
                    if (verbose) errs() << "I due loop hanno lo stesso Trip Count esatto (Puntatori SCEV identici)!\n";
                } 
                // Fallback: chiediamo a SCEV di fare la sottrazione algebrica. 
                // Se il risultato è la costante 0, i trip count sono identici.
                else {
                    ...
                    if (Difference->isZero()) {
                        ...
                        if (verbose) errs() << "I Trip Count differiscono.\n";
                        
                        // EXTRA: Se vuoi sapere se differiscono solo per una costante 
                        // (es. Loop 1 gira N volte, Loop 2 gira N-1 volte)
                        if (isa<SCEVConstant>(Difference)) {
                            if (verbose) errs() << "  -> Nota: Differiscono solo per un valore costante.\n";
                        }
                    }
                }
            }
